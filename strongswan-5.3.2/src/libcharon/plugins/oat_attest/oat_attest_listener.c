...
void update_cert_info(char *common_name, char cert_digest[41])
{
	...
	fp=fopen("/etc/strongswan/strongswan.d/charon/oat-attest.data", "w");
	if (fp == NULL)
	      exit(1);
...
int oat_attest_request(char *peer_hostname, char *peer_cert_digest)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (strcmp(json_object_get_string(host_name), peer_hostname) == 0) {
			...
			/* check the attestation result, if unknown, the client will
			skip the first two results, because the connection to the
			verifier maybe disabled with starting the PSA/PSC */
